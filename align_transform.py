...
class Align():
    ...
    def warp_image(self, source, target, M):
        ...
        merge = (np.sqrt(target) + np.sqrt(warp)) * 8
        # Show the result
    ...
    def align_image(self):
        ...
        merge = self.warp_image(img_source, img_target, M)
        # ~~~~~~~~~~~~~~~delete two sentence below~~~~~~~~~~~~~~~
        self.img_source = img_source
        self.img_taret = img_target
        return merge, M

    def show_with_points(self, M, nir_key, box):
        res = self.point_affine(M, nir_key)
        box = self.point_affine(M, box)
        L = len(res) / 2
        for i in range(L):
            x, y = res[2 * i], res[2 * i + 1]
            cv2.circle(self.img_source, (int(x), int(y)), 1, (0, 0, 255), 2)
        x1, y1, x2, y2 = map(int, box)
        cv2.rectangle(self.img_source, (x1, y1 + 4), (x2, y2), (255, 0, 0), 2)
        cv2.imwrite(self.source_path + 'warp.jpg', self.img_source)

    def point_affine(self, M, keypoints):
